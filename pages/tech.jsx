      ...
      <main className="max-w-screen max-w-5xl mx-auto">
        <div className="pt-24 flex flex-col mx-auto">
          ...
          <p className="mt-8 font-normal lg:text-lg text-center md:w-[70%] mx-auto text-base md:text-[1.15rem]">
            Just like any other dev, I also spend more time searching and experimenting with tools than actually coding some sensible things 😎
          </p>
        </div>
        ...
        <div className="mt-12 flex justify-center space-x-4">
          <button
            className={`px-4 py-2 rounded-lg ${
              activeSection === "web" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"
            }`}
          ...
          <button
            className={`px-4 py-2 rounded-lg ${
              activeSection === "mobile" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"
            }`}
          ...
          <button
            className={`px-4 py-2 rounded-lg ${
              activeSection === "ml" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"
            }`}
        ...
        <div className="flex mx-auto justify-between mt-12 space-x-4 md:space-x-6 lg:space-x-auto items-center">
          ...
          <motion.div
            className="bg-gradient-to-tr dark:from-[#dbfefe25] dark:via-[#f9f2ff10] dark:to-[#fff2ec20] from-[#dbfefe] via-[#f9f2ff] to-[#fff2ec] flex p-2 py-2 sm:p-3 md:p-4 lg:p-6 rounded-xl sm:rounded-[0.8rem] md:rounded-[1.2rem] lg:rounded-[1.6rem] mx-auto items-center justify-center space-x-2 sm:space-x-3 md:space-x-4 h-24 md:h-max lg:space-x-6 transition-all duration-700 ease-linear"
            whileHover={{ scale: 1.02 }}
          >
            <div className="basis-[30%] lg:basis-[25%] h-16 w-12 sm:h-20 sm:w-20 md:h-28 md:w-28 lg:h-32 lg:w-32 rounded-2xl">
              <div className="flex mx-auto rounded-2xl relative w-full h-full bg-white overflow-hidden">
                <motion.img
            ...
            <motion.div
              className="basis-[70%] lg:basis-[75%] mx-auto transition-all duration-500"
              whileHover={{ scale: 1.02 }}
            >
              <div className="flex mb-1 justify-between items-center">
                <motion.h3
                  className="text-[0.7rem] sm:text-sm md:text-base lg:text-xl font-semibold font-deca tracking-wide"
                  whileHover={{ scale: 1.1 }}
                >
                  {data[idNumber].title}
                </motion.h3>
                <motion.div
                  className={`${data[idNumber].bg} ${data[idNumber].text} text-[0.45rem] sm:text-[0.6rem] md:text-xs sm:px-1 lg:px-2 rounded-xl h-max font-bold`}
                  whileHover={{ scale: 1.1 }}
                >
                  {data[idNumber].tag}
                </motion.div>
              ...
              <motion.p
                className="text-[0.58rem] sm:text-[0.7rem] md:text-[0.9rem] md:text-base lg:text-[1.015rem] font-normal md:w-[90%]"
                whileHover={{ scale: 1.1 }}
              >
                {data[idNumber].desc}
              </motion.p>
